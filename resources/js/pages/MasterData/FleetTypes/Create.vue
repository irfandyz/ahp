<template>
  <AppLayout title="Create Fleet Type">
    <div class="space-y-6 p-5">
      <!-- Header -->
      <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
        <div>
          <h1 class="text-2xl font-bold">Create Fleet Type</h1>
          <p class="text-muted-foreground">Add a new fleet type for shipping</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Fill in the details below to create a new fleet type.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="flex justify-end space-x-3">
              ...
              <Button type="submit" :disabled="form.processing" class="px-4 py-2 bg-black text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                ...
                {{ form.processing ? 'Creating...' : 'Create Fleet Type' }}
              </Button>
            </div>
          </form>
        </CardContent>
...
const submit = () => {
  form.post(route('fleet-types.store'))
}
</script>
