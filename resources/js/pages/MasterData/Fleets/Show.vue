<template>
  <AppLayout title="Fleet Vehicle Details">
    <div class="space-y-6 p-5">
      <!-- Header -->
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold">Fleet Vehicle Details</h1>
          <p class="text-muted-foreground">View vehicle information and associated expeditions</p>
        </div>
        <div class="flex space-x-3">
          <Link :href="route('fleets.edit', fleet.id)" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          ...
          <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div class="space-y-4">
              <div>
                ...
                <p class="text-lg font-semibold">{{ fleet.plate_number }}</p>
              </div>
              <div>
                <Label class="text-sm font-medium text-gray-500">Fleet Type</Label>
                <p class="text-base">{{ fleet.fleet_type?.name || 'Not assigned' }}</p>
              </div>
            </div>
            <div class="space-y-4">
              <div>
                <Label class="text-sm font-medium text-gray-500">Description</Label>
                <p class="text-base">{{ fleet.description || 'No description provided' }}</p>
              </div>
            </div>
          </div>
          <div class="mt-6 pt-6 border-t border-gray-200">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                ...
                <p class="text-base">{{ formatDate(fleet.created_at) }}</p>
              </div>
              <div>
                <Label class="text-sm font-medium text-gray-500">Last Updated</Label>
                <p class="text-base">{{ formatDate(fleet.updated_at) }}</p>
              </div>
              <div>
                <Label class="text-sm font-medium text-gray-500">Total Expeditions</Label>
                <p class="text-base font-semibold">{{ fleet.expeditions?.length || 0 }}</p>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card v-if="fleet.expeditions && fleet.expeditions.length > 0">
        <CardHeader>
          <CardTitle>Recent Expeditions</CardTitle>
          <CardDescription>
            Latest expedition assignments for this vehicle
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="expedition in fleet.expeditions" :key="expedition.id" class="border-b hover:bg-gray-50">
      ...
      <Card v-else>
        <CardContent class="text-center py-8">
          ...
          <p class="text-gray-500">This vehicle hasn't been assigned to any expedition operations yet.</p>
        </CardContent>
      </Card>
    ...
    <Dialog v-model:open="showDeleteDialog">
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete "{{ fleet.plate_number }}"? This action cannot be undone.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
...
const confirmDelete = () => {
  ...
  router.delete(route('fleets.destroy', props.fleet.id), {
    onFinish: () => {
