<template>
  <AppLayout :title="`Create Goods - ${expedition.order_number}`">
    <div class="space-y-6 p-5">
      <!-- Header -->
      <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Create Goods</h1>
      ...
      <form @submit.prevent="submitForm" class="space-y-6">
        ...
        <div class="flex justify-end space-x-3">
          ...
          >
            ...
            {{ isSubmitting ? 'Saving...' : 'Save Goods' }}
          </Button>
        </div>
      </form>
    </div>
...
const submitForm = () => {
  ...
  router.post(route('expeditions.goods.store', props.expedition.id), form.value, {
    onFinish: () => {
      isSubmitting.value = false
    }
  })
